...
import com.io7m.stonesignal.protocol.data.v1.St1DataMessageType;
...
abstract class St1BearerHandler implements Handler
{
  ...
  {
    if (!Objects.equals(this.token, this.configuration.dataAPI().apiKey())) {
    ...
    final ServerResponse response)
  {
    this.send(
      response,
      new St1DataError(exception.errorCode(), exception.getMessage())
    );
  }
    ...
    final ServerResponse response)
  {
    ...
    this.send(response, new St1DataError("error-general", message));
  }

  protected final <C extends St1DataMessageType> C read(
    final ServerRequest request,
    final Class<C> clazz)
  {
    try {
      try (final var stream = request.content().inputStream()) {
        return switch (this.format) {
          case JSON ->
            St1DataProtocolMapperJSON.get().readValue(stream, clazz);
          case CBOR ->
            St1DataProtocolMapperCBOR.get().readValue(stream, clazz);
        };
      }
  ...
  protected final void send(
    final ServerResponse response,
    final St1DataMessageType message)
  {
    try {
      response.send(
        switch (this.format) {
          case JSON ->
            St1DataProtocolMapperJSON.get().writeValueAsBytes(message);
          case CBOR ->
            St1DataProtocolMapperCBOR.get().writeValueAsBytes(message);
        }
      );
